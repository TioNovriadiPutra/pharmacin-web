...
const AuthInputBox = ({ formData, type }) => {
  ...
  return (
    <div
      className={`${type === "login" ? "w-97" : "w-100"} ${
        type === "login" ? "py-10.5" : "py-8"
      } px-11.5 flex-1 bg-white rounded-md max-h-100`}
    >
      <div
        className={`items-center text-center gap-2 mx-12 ${
          type === "login" ? "mb-18" : "mb-12"
        }`}
      >
      ...
      <form
        onSubmit={handleSubmit(type === "login" ? useLogin : useRegisterAdmin)}
        className="flex-1"
      >
        <div className="flex-row gap-12.5">
          {formData.formData.map((inputPage, index) => (
            <ScrollContainer key={index.toString()} styles="gap-6">
                    ...
                    <TextInput
                      ...
                      validationError={
                        ...
                        validationError.find(
                          (item) => item.field === inputData.name
                        )
                      }
                    />
                  );
        ...
        <SubmitButton
          buttonData={formData.submitButton}
          color={type === "login" ? "bg-primary" : "bg-secondary"}
          styles={`${type === "login" ? "mt-18" : "mt-5"} px-26`}
        />
      </form>
